define(function () {
    ...
    util.wholeDistance = function (points) {
        var distance = 0;
    ...
    util.getBaseLength = function (points) {
        return Math.pow(util.wholeDistance(points), 0.99);
    //return util.wholeDistance(points);
    };
    
    util.mid = function (pnt1, pnt2) {
        return [(pnt1[0] + pnt2[0]) / 2, (pnt1[1] + pnt2[1]) / 2];
    };
    
    util.getCircleCenterOfThreePoints = function (pnt1, pnt2, pnt3) {
        ...
        return util.getIntersectPoint(pntA, pntB, pntC, pntD);
    };
    
    util.getIntersectPoint = function (pntA, pntB, pntC, pntD) {
        if (pntA[1] == pntB[1]) {
    ...
    util.getAzimuth = function (startPnt, endPnt) {
        var azimuth;
        var angle = Math.asin(Math.abs(endPnt[1] - startPnt[1]) / util.distance(startPnt, endPnt));
    ...
    util.getPointOnLine = function (t, startPnt, endPnt) {
        var x = startPnt[0] + (t * (endPnt[0] - startPnt[0]));
        var y = startPnt[1] + (t * (endPnt[1] - startPnt[1]));
        return [x, y];
    };
    
    util.getCubicValue = function (t, startPnt, cPnt1, cPnt2, endPnt) {
    ...
    util.getThirdPoint = function (startPnt, endPnt, angle, distance, clockWise) {
        var azimuth = util.getAzimuth(startPnt, endPnt);
    ...
    util.getBisectorNormals = function (t, pnt1, pnt2, pnt3) {
    ...
    util.getNormal = function (pnt1, pnt2, pnt3) {
    ...
    util.getCurvePoints = function (t, controlPoints) {
        var leftControl = util.getLeftMostControlPoint(controlPoints);
        ...
        for (var i = 0; i < controlPoints.length - 2; i++) {
            ...
            var normalPoints = util.getBisectorNormals(t, pnt1, pnt2, pnt3);
            normals = normals.concat(normalPoints);
        }
        var rightControl = util.getRightMostControlPoint(controlPoints);
    ...
    util.getLeftMostControlPoint = function (controlPoints) {
        var pnt1 = controlPoints[0];
        var pnt2 = controlPoints[1];
        var pnt3 = controlPoints[2];
        var pnts = util.getBisectorNormals(0, pnt1, pnt2, pnt3);
    ...
    util.getRightMostControlPoint = function (controlPoints) {
        ...
        if (dist > P.Constants.ZERO_TOLERANCE) {
            ...
            var d1 = util.distance(pnt2, pnt3);
            // normal at midpoint
            var n = 2.0 / d1;
            var nX = -n * pY;
            var nY = n * pX;
    ...
    util.getBezierPoints = function (points) {
        if (points.length <= 2)
            return points;
        ...
        for (var t = 0; t <= 1; t += 0.01) {
            ...
            for (var index = 0; index <= n; index++) {
                var factor = util.getBinomialFactor(n, index);
    ...
    util.getBinomialFactor = function (n, index) {
        return util.getFactorial(n) / (util.getFactorial(index) * util.getFactorial(n - index));
    };
    
    util.getFactorial = function (n) {
        if (n <= 1)
            return 1;
        if (n == 2)
            return 2;
    ...
    util.getQBSplinePoints = function (points) {
        if (points.length <= 2)
            return points;
        ...
        for (var i = 0; i <= m; i++) {
            for (var t = 0; t <= 1; t += 0.05) {
                var x = y = 0;
                for (var k = 0; k <= n; k++) {
                    var factor = util.getQuadricBSplineFactor(k, t);
    ...
    util.getQuadricBSplineFactor = function (k, t) {
        if (k == 0)
